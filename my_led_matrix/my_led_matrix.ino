...
// HARDWARE SPI - use one of the values from below:
// PAROLA_HW
// GENERIC_HW
// ICSTATION_HW
// FC16_HW
...
// MD_Parola P = MD_Parola(MD_MAX72XX::GENERIC_HW, CS_PIN, MAX_DEVICES);
...
void setup() {
  ...
  PRINT("\nConnecting to ", ssid);
  sprintf(curMessage, "%s %s Network", "Connecting to ", ssid);
...
void reconnect() {
  if (!client.connected()) {
    ...
    P.displayScroll("Connecting to MQTT Server", PA_CENTER, scrollEffect, FRAME_DELAY_DEFAULT);
    waitForMessageComplete(false);
